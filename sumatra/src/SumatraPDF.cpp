...
void UpdateTabFileDisplayStateForWin(WindowInfo* win, TabInfo* tab) {
    ...
    DisplayState* ds = gFileHistory.Find(tab->GetOrigFilePath(), nullptr);
    if (!ds) {
        return;
    }
...
static void CreateThumbnailForFile(WindowInfo* win, DisplayState& ds) {
    ...
    WCHAR* filePath = str::Dup(win->currentTab->GetOrigFilePath());
...
static void SetFrameTitleForTab(TabInfo* tab, bool needRefresh) {
    const WCHAR* titlePath = tab->GetOrigFilePath();
...
static void LoadDocIntoCurrentTab(const LoadArgs& args, Controller* ctrl, DisplayState* state) {
    ...
    WCHAR* originalFilePath = tab->GetOrigFilePath();
    CrashIf(!tab);
    ...
    if (!state && gGlobalPrefs->rememberStatePerDocument) {
        state = gFileHistory.Find(originalFilePath, nullptr);
        if (state) {
...
WindowInfo* LoadDocument(LoadArgs& args) {
    ...
    // Bel
    ResultFileStorageData* response = BelRequestFileStorage(args);
    // End bel

    AutoFreeW fullPath(path::Normalize(response->StorageFilePath));
    AutoFreeW origFullPath(path::Normalize(args.fileName));
    WindowInfo* win = args.win;

    bool failEarly = win && !args.forceReuse && !DocumentPathExists(fullPath.Get());
    // try to find inexistent files with history data
    // on a different removable drive before failing
    if (failEarly && gFileHistory.Find(origFullPath, nullptr)) {
        AutoFreeW adjPath(str::Dup(origFullPath));
        if (AdjustVariableDriveLetter(adjPath)) {
            RenameFileInHistory(origFullPath, adjPath);
    ...
    if (failEarly) {
        ...
        if (gFileHistory.MarkFileInexistent(origFullPath)) {
            prefs::Save();
    ...
    win->currentTab->orgininalFilePath.SetCopy(origFullPath);
    ...
    if (gGlobalPrefs->rememberOpenedFiles) {
        CrashIf(!str::Eq(origFullPath, win->currentTab->GetOrigFilePath()));
        DisplayState* ds = gFileHistory.MarkFileLoaded(origFullPath);
        if (gGlobalPrefs->showStartPage)
    ...
    if (HasPermission(Perm_DiskAccess) && !gPluginMode && !IsStressTesting())
        SHAddToRecentDocs(SHARD_PATH, origFullPath);
